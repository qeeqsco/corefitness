        ...
        $(this).each(function() {
            ...
            this.renderParallax = function() {
                if (options.slideSize.type === 'full') {
                    adv.find('.parallax-block-content').css({
                        'width': adv.parent().parent().width(),
                        'marginLeft': adv.parent().parent().offset().left
                    });
                }
            ...
            this.openParallaxContent = function(e) {
                ...
                adv.css(cssOpen).animate({
                        'top': btParallax.eTopCenter
                    }, {
                        duration: espeed,
                        step: function(now, fx) {
                            var walk = Math.round(((now - btParallax.eTopCenter) / s) * btParallax.back_posY);
                            adv.find('.parallax-background').css({
                                left: btParallax.back_posX + 'px',
                                top: walk + 'px'
                            });
                        },
                        done: function() {
            ...
            this.openParallaxContentLoad = function() {
                ...
                if (options.contentType === 'video') {
                    ...
                    adv.find('.parallax-content-wrap').css({
                        'overflow': 'hidden'
                    });
                    adv.find('.content-show-large').css({
                ...
                adv.find('.parallax-content-wrap').fadeIn(500, function() {
                    ...
                    if (options.contentType === 'video') {
                        ...
                        adv.find('.content-show-large .loading').css({
                            'position': 'absolute',
                            'top': '50%',
                            'left': '50%',
                        ...
                        adv.find('.content-show-large .item-contain video:first').on('loadedmetadata.resize', function() {
                            adv.find('.content-show-large .item-contain video').each(function() {
                                if ($(window).width() * $(this).height() / $(this).width() < $(window).height()) {
                                    ...
                                    $(this).css({
                                        'margin-left': (($(window).width() - $(this).width()) / 2) + 'px',
                                        'margin-top': 0
                                    });
                                } else {
                                    $(this).css({
                                    ...
                                    $(this).css({
                                        'margin-top': (($(window).height() - $(this).height()) / 2) + 'px',
                                        'margin-left': 0
                                    });
                                }
                            });
            ...
            this.closeButtonClick = function(e) {
                if ($(e).hasClass('close-image-info')) {
                    ...
                    adv.find('.content-show-large.show').animate({
                        'top': btParallax.ePos.top,
                        'left': btParallax.ePos.left,
                        'width': 0,
                        'height': 0
                    }, 500, function() {
            ...
            this.parallaxContentZoomOutWidth = function(callback) {
                adv.animate({
                        'left': (btParallax.win_width - btParallax.para_width) / 2,
                        'width': btParallax.para_width
                    }, {
                        duration: 500,
                        step: function(now, fx) {
            ...
            this.navNextClick = function(e) {
                if ($(e).hasClass('next-large')) {
                    ...
                    if (left + options.next_prev_s <= btParallax.c_pos.width() - adv.find('.parallax-content.post-content').width()) {
                        btParallax.c_pos.animate({
                            'transform': 'translateX(-' + (left + options.next_prev_s) + 'px)'
                        }, 400, function() {
                    ...
                    btParallax.c_pos.animate({
                        'transform': 'translateX(' + (btParallax.c_pos.position().left - options.next_prev_s) + 'px)'
                    }, 400, function() {
                        btParallax.isNavClick = true;
                        btParallax.animateStart();
                    });
            ...
            this.navPrevClick = function(e) {
                if ($(e).hasClass('prev-large')) {
                    ...
                    var n_e = $(ne).val();
                    adv.find('.content-show-large .item-contain').append($(n_e).css({
                        'display': 'none'
                    }));
                    if (adv.find('.content-show-large .item-contain .video-inner').length > 2) {
                        adv.find('.content-show-large .item-contain .video-inner:first').remove();
                    ...
                    if (left - options.next_prev_s >= 0) {
                        ...
                        btParallax.c_pos.animate({
                            'transform': 'translateX(-' + (btParallax.c_pos.width() - btParallax.c_pos.parent().width() - options.spacing) + 'px)'
                        }, 400, function() {
                            btParallax.isNavClick = true;
                        });
                    }
                ...
                if (btParallax.isThumClick === false) {
                    ...
                    $(img).load(function() {
                        ...
                        adv.find('.content-show-large').css({
                            ...
                            'width': 0,
                            'height': 0
                        }).removeClass('hidden').addClass('show').animate({
                            'top': 0,
                            'left': 0,
                            'width': '100%',
                            'height': '100%'
            ...
            this.backgroundImageResize = function(h) {
                if (h) {
                    if (btParallax.background_img.height() < btParallax.background.height()) {
                        btParallax.background_img.css({
                            'width': 'auto',
                            'height': btParallax.background.height()
                        });
                    }
                } else {
                    if (btParallax.background_img.width() < btParallax.background.width()) {
                        btParallax.background_img.css({
                            'width': btParallax.background.width(),
                            'height': 'auto'
                        });
                    }
                }
            ...
            this.responsive = function() {
                ...
                if (adv.parent().hasClass('full-width')) {
                    adv.parent().css({
                        'width': btParallax.win_width,
                        'marginLeft': '-' + adv.parent().parent()[0].getBoundingClientRect().left + 'px'
                    });
                    adv.find('.parallax-block-content').css({
                        'width': adv.parent().parent().width(),
                        'marginLeft': adv.parent().parent().offset().left
                ...
                if (options.contentType === 'video') {
                    adv.find('.item-contain').css({
                        'height': btParallax.win_height,
                        'width': btParallax.win_width
                    });
                }
            };
